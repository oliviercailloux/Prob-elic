...
\begin{conjecture}
	\label{th:limDelta}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$, define the sequence $p_\delta$, for $\delta \in \N$, as $p_\delta = P[z < b \knowing {\ppref}]$, with the partial order $\ppref$ being defined as follows: $A \ppref b \ppref C = \set{c, …}$, $D \ppref c$, with disjoint sets $A$, $B = \set{b}$, $C$ and $D$, and with $\alpha = \card{A}$, $\gamma = \card{C} ≥ 1$ and $\delta = \card{D}$.
...
\begin{proof}
	...
	We can restrict to considering the complete orderings $\pref$ where the objects composing $A$ and $D$ conform to an arbitrary fixed order $a_1 \pref a_2 \pref …$ and $d_1 \pref d_2 \pref …$. 

	Let us condition on $i$, the number of alternatives among $D$ below $b$ in $\pref$. 
	
	To count the number of orderings $\pref$ extending $\ppref$ such that $i$ objects among $D$ (say, $D_1 = \downa[b] \cap D$) are below $b$, $\card{D_1} = i$, observe that there are $\binom{i + \gamma}{\gamma - 1}$ ways of placing $C \setminus \set{c}$ (the “moving” objects) among the $\card{\downa[b]} = i + \gamma$ objects below $b$; and there are $\binom{\alpha + (\delta - i)}{\alpha}$ ways of ordering the objects in $A \cup (\upa[b] \cap D)$ above $b$.
...
\begin{conjecture}
	\label{th:limDeltaAll}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$, define the sequence $p_\delta$, for $\delta \in \N$, as $p_\delta = P[z < b \knowing {\ppref}]$, with the partial order $\ppref$ being defined as follows: $A \ppref b \ppref C$, $D \ppref C$, with disjoint sets $A$, $B = \set{b}$, $C$ and $D$, and with $\alpha = \card{A}$, $\gamma = \card{C} ≥ 1$ and $\delta = \card{D}$.
	
	The sequence $p_\delta$ is monotonically decreasing, and converges to:
	\begin{equation}
		\lim_{\delta → ∞} P[z < b \knowing {\ppref}] = \frac{1}{\alpha + 1},
	\end{equation}
\end{conjecture}
\begin{proof}
	Let us condition on $i$, the number of alternatives among $D$ below $b$ in $\pref$. 
	
	To count the number of orderings $\pref$ extending $\ppref$ such that $i$ objects among $D$ (say, $D_1 = \downa[b] \cap D$) are below $b$, $\card{D_1} = i$, observe that there are $\binom{i + \gamma}{\gamma - \gamma}$ ways of placing $C \setminus C$ (the moving objects) among the $\card{\downa[b]} = i + \gamma$ objects below $b$; and there are $\binom{\alpha + (\delta - i)}{\alpha}$ ways of ordering the objects in $A \cup (\upa[b] \cap D)$ above $b$.
