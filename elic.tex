...
Select a pivot element randomly (equiprobably). Compare every other elements to the pivot. Run again on either the lower elements or the higher elements, depending on where the median lies, or stop if a median has been found. 

Let $V(n)$ be the number of comparisons required to run this algorithm, and let us compute an upper bound for $E[V(n)]$.
...
That bound is tight for $1 ≤ n ≤ 2$, as $V(1) = 0$ and $V(2) = 1$. Also, $E[V(3)] = 2 + \frac{2}{3}$ and $E[V(4)] = 3 + \frac{1}{2}E[V(3)] = 4 + \frac{1}{3}$.
