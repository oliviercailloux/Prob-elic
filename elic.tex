...
\begin{theorem}[XYZ]
	Consider $b, d, z \in X$ and any poset ${\ppref} \in \POs$ such that $d \nppref b$. We have:
	\begin{equation}
		P(b \ppref z \knowing {\ppref}) ≤ P(b \ppref z \knowing {\ppref}, b \ppref d).
	\end{equation}
\end{theorem}
...
\begin{proof}[Rewrite of Shepp 1982]
	...
	Define $S^\mathit{fav} \subseteq S^\mathit{sat}$ as the subset of functions that also satisfy $b \ppref z$, thus, $s(b) > s(z)$.
	Define $S^\mathit{cond} \subseteq S^\mathit{sat}$ as the subset of functions that satisfy $\ppref$ and also $b \ppref d$, thus, $s(b) > s(d)$.
	...
	Compared to $s \in S^\mathit{sat}$, $t = \tau(s)$ negates the score of $b$ and associates each other object to its difference of score compared to $b$, that difference being positive iff the object is preferred to $b$ according to $s$.
	Note that, given $t \in \intvl{-(n - 1), n - 1}^X$, and letting $s$ denote $\tau^{-1}(t)$, we have $t \in T ⇔ [\forall x \ppref y: s(x) - s(b) > s(y) - s(b)]$. Thus: 
	\begin{equation}
		\label{eq:tsat}
	...
	Note as well that going upwards in the lattice $<^\mathit{p}$ preserves the properties of being “fav”, in other words, $\forall t \in T^\mathit{fav}: t <^\mathit{p} t' ⇒ [\forall t' \in T: t' \in T^\mathit{fav}]$. Indeed, given any $t \in T$, $t \in T^\mathit{fav} ⇔ t(z) < 0$, and if $t <^\mathit{p} t'$, then $t(z) ≤ t'(z)$.
	Similarly, going upwards in the lattice $<^\mathit{p}$ preserves the properties of being “cond”.
	
	We can now apply FKG, which states that if $<^\mathit{p}$ is a distributive lattice over $T$ and given $T^\mathit{fav}, T^\mathit{cond} \subseteq T$ such that $\forall t \in T^\mathit{fav}: t <^\mathit{p} t' ⇒ t' \in T^\mathit{fav}$, and similarly for $T^\mathit{cond}$; it holds that $\card{T^\mathit{fav}} × \card{T^\mathit{cond}} ≤ \card{(T^\mathit{fav} \cap T^\mathit{cond})} × \card{T}$.
	
	Rewriting, we obtain that $\frac{\card{T^\mathit{fav}}}{\card{T}} ≤ \frac{\card{(T^\mathit{fav} \cap T^\mathit{cond})}}{\card{T^\mathit{cond}}}$.
\end{proof}
