...
\begin{figure}
	\begin{tikzpicture}
		...
		\path (tree0.south) ++ (0, -1cm) node[anchor = north] (root abc) {$a > b > c$, E[] = \pgfmathparse{(6 * 2 + 4 * 10) / 12}\pgfmathresult}
		...
		\path (root abc) ++ (0, -5.5cm) [sibling distance = 3cm, level 2/.style = {sibling distance = 1.5cm, align = left, anchor = north}] node[anchor=north] (root ac b) {$\set{a, c} > b$}
			child {
				node {$a \relq c$}
				child {
					node {
