...
\begin{conjecture}
Assume > has a single connex component, and the maximal size of a chain is 3. Assume b is the middle element of every chains of size 3. Then the optimal strategy asks b with a new element.
\end{conjecture}
...
\begin{conjecture}
Assume > has a single connex component, and every undirected path has at most three objects. Assume b is the middle element of every chains of size 3, and some element $f$ is not part of the component. Then asking $q_{bf}$ is an optimal strategy.
\end{conjecture}
\begin{proof}
	Let’s check first that this is myopically optimal. Consider a chain $abc$.
	\[E[q_{af}] = P(f > a \knowing >) ({>}(b) + 1),\]
	\[E[q_{bf}] = P(f > b \knowing >) {>}(b) + P(b > f \knowing >) {>}^{-1}(b).\]
	...
	\[[P(f > b \knowing >) - P(f > a \knowing >)] {>}(b) ≥ 2 P(f > b \knowing >) − 1.\]
	
	I also believe that:
	\[[P(f > b \knowing >) = \frac{(1 + {>}(b))! {>}^{-1}(b)!}{P(>)}.\]
\end{proof}
...
Start by using two questions to order $\set{a, b, c}$ \commentOC{not wlog!}. If $>_2$ is a linear order (say, $abc$), spend the remaining two questions by comparing yet unknown elements with $b$. Otherwise (say, $\set{a, c} > b$), disconnect temporarily by asking $q_{de}$ (say, $de$), then connect the two components by asking $q_{be}$.
...
\begin{figure}
	\begin{tikzpicture}
		...
		\path (abc-bd-node.south) ++(-7mm, -1cm) coordinate (abc-bd-be);
		...
		\path (abc-bd-node.south) ++(7mm, -1cm) coordinate (abc-bd-eb);
		\path (abc-bd-eb) ++(-3mm, 0) node[anchor=north] (abc-bd-eb-a) {$a$};
		\path (abc-bd-eb-a.south) ++(3mm, -\ordown) node[anchor=north] (abc-bd-eb-b) {$b$};
		\path (abc-bd-eb-b.south) ++(-3mm, -\ordown) node[anchor=north] (abc-bd-eb-c) {$c$};
		\path (abc-bd-eb-c.base) ++(6mm, 0) node[anchor=base, inner sep=2pt] (abc-bd-eb-d) {$d$};
		\path (abc-bd-eb-a.base) ++(6mm, 0) node[anchor=base] (abc-bd-eb-e) {$e$};
		\path [draw] (abc-bd-eb-a) -- (abc-bd-eb-b) -- (abc-bd-eb-c);
		\path [draw] (abc-bd-eb-b) -- (abc-bd-eb-d);
		\path [draw] (abc-bd-eb-e) -- (abc-bd-eb-b);
		\path [draw, color=red] (abc-bd-eb-e) to[out=-90, in=60] (abc-bd-eb-c);
		\path [draw, color=red] (abc-bd-eb-e) to[out=-45, in=60] (abc-bd-eb-d);

		\path (abc-bd-node.south)  edge node[left] {$b > e$} (abc-bd-be);
		\path (abc-bd-node.south) edge node[right] {$e > b$} (abc-bd-eb);		
		
		\path (ab-cb-de-south) ++(-7mm, -1cm) coordinate (abe-cb-de);

		\path (ab-cb-de-south) ++(7mm, -1cm) coordinate (ab-cb-deb);
...
Start by using two questions to order $\set{a, b, c}$. \commentOC{Not wlog!}
