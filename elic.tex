...
\begin{proof}
	...
	We can associate a questioning strategy $s'$ to a probability distribution over possible states, where a state associates to each $x \in X$ a pair $(A, C)$, the number of objects known better than $x$ and known worst than $x$.
	Define $s$ as the questioning strategy that asks $q_{bf}$ with a new element $f$ at each step.
	
	We will show that the expected gain of the next question is not worst for $\ppref$ with a state $b → (A, C)$ than for $\ppref'$ with a state $a → (A', C')$, where $A + C ≥ A' + C'$ and $\abs{A - C} ≤ \abs{A' - C'}$ and $>$ has all elements in $A$ uncomparable and those in $C$ incomparable.
	
	We will then show that given $s'$, we can partition the outcomes $O'$ so that each $O'_i$ is associated to a set of outcomes $O_i$ that are each better (in exp of next question) than any of $O'_i$ and such that the prob of having $O_i$ or better for $s$ is not lower than the prob of having $O'_i$ or better for $s'$.
	
	This shows that $s$ is better, in terms of exp of the next question, in expectation, than $s'$. (That’s the expectation of the gain of the $k+1$th question.)
	
	Given ${\ppref}$ and a strategy $s'$, let ${\ppref}_k^{s'}$ designate the random variable that represents our (increased) knowledge after $k$ questions following $s'$.
	...
	Let $l = \card{{\pprefinv}(b)} + \card{{\ppref}(b)}$.
	
	To any $0 ≤ d ≤ k + l$, associate to the set $\set{{\ppref}_k^{s'} \suchthat \card{{\pprefinv}(a)} + \card{{\ppref}(a)} = k + l, \abs{\card{{\pprefinv}(a)} - \card{{\ppref}(a)}} ≤ d}$ the set $\set{{\ppref}_k^{s} \suchthat \card{({\ppref}_k^{s'})^{-1}(b)} + \card{{\ppref}(b)} = k + l, \abs{\card{{\pprefinv}(b)} - \card{{\ppref}(b)}} ≤ d}$.
