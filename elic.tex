...
\begin{proof}
	...
	%Define also $g(q)$ as a random variable that associates to any state ${\ppref}$ (considered as a set of linear orders) and ${\pref}$ the gain $g(q, {\ppref}, {\pref})$.
	...
	Define $S$ the preorders reachable after any $k$ questions, starting from a preorder $\ppref$ as described in the conjecture.
	
	Define $\ppref'$ (a random variable) as $\ppref$ followed by $k$ questions about $b$ and new elements.

	We will define a bijection sending $>_1 \in S$ to $>_1' \in S$, so that $P(>_1' \knowing {\ppref}, k) ≤ P(>_1 \knowing {\ppref}, k)$ and $E[g(q_{bf}, {\ppref'}, {\pref}) \knowing {\ppref'}] - E[g(q_{af}, {>_1}, {\pref}) \knowing >_1] ≥ E[gain q_{af} \knowing >_1'] - E[gain q_{bf} \knowing {\ppref'}]$.
		
%	We will partition $S$ into $S_1, S_1', S_2, S_2', …$ (disjoint and covering $S$), so that $P(S_1' \knowing {\ppref}, k) ≤ P(S_1 \knowing {\ppref}, k)$ and $E[g(q_{bf}, {\ppref'}, {\pref}) \knowing {\ppref'}] - E[g(q_{af}, {S_1}, {\pref}) \knowing S_1] ≥ E[gain q_{af} \knowing S_1'] - E[gain q_{bf} \knowing {\ppref'}]$.
	The latter expression is equivalent to $\frac{E[]}{}$.
\end{proof}
