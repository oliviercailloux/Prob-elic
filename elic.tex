...
\begin{conjecture}[Alternative writing of the more general, worst case]
	...
	Let ${\ppref_1} \in \POs$ with $A × B \subseteq {\ppref_1}$, $B × C \subseteq {\ppref_1}$, ${\pprefeq_1^{± 1}}(X') = X'$ (thus $X'$ being isolated from $X \setminus X'$ in $\ppref_1$).
	
	For any $c \in C, d \in D, z \in X \setminus X'$, letting ${\ppref_2} = {\ppref} \cup \set{(d, c)}$, we have:
	\begin{equation}
...
\begin{proof}
	...
	Define $m = \card{X'}$.
	Let $r(b) \in \intvl{1, m}$ denote the rank of $b$ among $X'$ given some complete ordering over $X$.
	
	Note that for ${\ppref} \in \set{{\ppref_1}, {\ppref_2}}$:
	\begin{align}
		P[z > b \knowing {\ppref}] & = \sum_{1 ≤ k ≤ m} P[r(b) = k \knowing {\ppref}] P[z > b \knowing {\ppref} \land r(b) = k]\\
	...
	We have to establish that, for any $k \in \intvl{1, m}$:
	\begin{equation}
		P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(c, d)}] ≤ P[r(b) ≥ k \knowing {\ppref_1}].
	\end{equation}
	
	This entails that $P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(d, c)}] ≥ P[r(b) ≥ k \knowing {\ppref_1}]$, as the RHS is a weighted average of $P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(c, d)}]$ and $P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(d, c)}]$. 

	This therefore establishes the stochastic dominance of $(r(b) \knowing {\ppref_2})$ over $(r(b) \knowing {\ppref_1})$, in turn leading to $E[r(b) \knowing {\ppref_2}] ≥ E[r(b) \knowing {\ppref_1}]$.
	
	Alternatively, we could establish that $P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(c, d)}] ≤ P[r(b) ≥ k \knowing {\ppref_1} \cup \set{(d, c)}]$, or directly that $E[r(b) \knowing {\ppref_2}] ≥ E[r(b) \knowing {\ppref_1}]$, or that $E[r(b) \knowing {\ppref_3}] ≤ E[r(b) \knowing {\ppref_1}]$ with ${\ppref_3} = {\ppref_1} \cup \set{(c, d)}$.
	...
	\begin{align}
		...
		\end{aligned}\right\}\\
		& = \sum_{{\ppref_1} \subseteq {\pref} \in \linors[X' \setminus \set{d}]} \left\{\begin{aligned}
			& P[\text{any order on } X' \knowing {\ppref_1}] r(b, {\pref}) (r(b, {\pref}) + 1) \\
			+ & P[\text{any order on } X' \knowing {\ppref_1}] (m - r(b, {\pref})) r(b, {\pref})
		\end{aligned}\right\}\\
		& = P[\text{any order on } X' \knowing {\ppref_1}] \sum_{{\ppref_1} \subseteq {\pref} \in \linors[X' \setminus \set{d}]} \left\{\begin{aligned}
			& r(b, {\pref}) (r(b, {\pref}) + 1) \\
			+ & (m - r(b, {\pref})) r(b, {\pref})
		\end{aligned}\right\}\\
		& = P[\text{any order on } X' \knowing {\ppref_1}] (m + 1) \sum_{{\ppref_1} \subseteq {\pref} \in \linors[X' \setminus \set{d}]} \left\{r(b, {\pref})\right\}
	\end{align}
	whereas
	\begin{align}
		E[r(b) \knowing {\ppref_1} \cup \set{(b, d)}] & = \sum_{({\ppref_1} \cup \set{(b, d)}) \subseteq {\pref} \in \linors[X' \setminus \set{d}]} P[{\pref} \knowing {\ppref_1} \cup \set{(b, d)}] r(b, {\pref}).
	\end{align}
	\footnote{Example with $A = \emptyset$, $C = \set{c_1, c_2}$, $D = \set{d_1, d_2}$, ${\ppref_1} = \set{(b, c_1), (b, c_2), (c_1, c_2), (d_2, c_2)}$.
		...
		This is equal to $\frac{6}{15} (2 + 1 + 1)$. How lucky.
	}
\end{proof}
