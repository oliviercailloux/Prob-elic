...
\begin{conjecture}
	Hopefully, for any ${\ppref} \in \POs$ and $b \in \allalts$ with potential $\zeta({\ppref}, b) = (s, -d)$ and ${\ppref'} \in \POs$ and $b' \in \allalts$ with potential $\zeta({\ppref'}, b') = (s', -d') < (s, -d)$, 
...
\end{conjecture}
\begin{proof}
A partial justification for this bold claim is that I think that $p$ decreases rapidly when the difference increases.
...
%	Define $\alpha = \card{\upa[b]}$ and $\gamma = \card{\downa[b]}$.
	...
	Define $p$ as the probability of a favorable answer to the question, that is, one that yields a gain of $\max(\upa, \downa)$, thus, $p = P[z < b \knowing {\ppref}]$ iff $\upa ≥ \downa$ and $p = P[z > b \knowing {\ppref}]$ otherwise.
	
	A question $q_{bz}$ yields as expected gain $E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = p \frac{s + d}{2} + (1 - p) \frac{s - d}{2}$.
	Observe that only $p$ depends on the configuration of $\ppref$, when $s$ and $d$ are fixed, and observe that minimizing (respectively, maximizing) $p$ is equivalent to minimizing (respectively, maximizing) the gain.
	
	Now, out of the many possible configurations of partial orders and objects with potential $(s, d)$, I conjecture that the one that minimises $p$, hence, the gain, is the one referenced by \cref{th:limDelta}. 
	If this is correct, we obtain, $\forall {\ppref} \in \POs, b \in \allalts \suchthat \zeta({\ppref}, b) = (s, -d)$:
	\begin{equation}
		E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] ≥ \frac{\gamma}{\gamma + \alpha + 1} \frac{s + d}{2} + \frac{\alpha + 1}{\gamma + \alpha + 1} \frac{s - d}{2}.
	\end{equation}
	(This quantity is the infimum of the gain among the partial orders and objects with that potential, when $n$ is considered unbounded.)
	
	For the upper bound, note that maximizing the gain is equivalent to minimizing $p'$, defined as $p' = P[z < b \knowing {\ppref}]$ iff $\upa ≤ \downa$ and $p' = P[z > b \knowing {\ppref}]$ otherwise.
	
	We compute the min and max possible gain at level $(s, -d)$. Then show that min possible gain $(s, -d)$ higher than max possible gain at $(s', -d')$.
	
\end{proof}

\begin{remark}
	Towards showing a sufficient condition for $q_{bz}$ to be an (non-myopically) optimal question, this remarks tries to argue that whenever a distribution “dominates” another one in some technical sense described here, asking $q_{bz}$ is optimal.
	...
	Use conjecture above to yield: a preorder ${\ppref}$ with potential $\zeta({\ppref}) = (s, -d)$ therefore has a not lower gain $g({\ppref})$ than another preorder ${\ppref'}$ with lower potential $\zeta({\ppref'}) = (s', d') < (s, -d)$.
