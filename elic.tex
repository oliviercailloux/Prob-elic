...
\begin{conjecture}
	\label{th:limDelta}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$, define the sequence $p_\delta$, for $\delta \in \N$, as $p_\delta = P[z < b \knowing {\ppref}]$, with the partial order $\ppref$ being defined as follows: $A \ppref b \ppref C = \set{c, …}$, $D \ppref c$, with $\alpha = \card{A}$, $\gamma = \card{C} ≥ 1$ and $\delta = \card{D}$.
	
	The sequence $p_\delta$ is monotonically decreasing, and converges to:
	\begin{equation}
		\lim_{\delta → ∞} P[z < b \knowing {\ppref}] = \frac{\gamma}{\gamma + \alpha + 1},
	\end{equation}
\end{conjecture}
\begin{proof}
	The intuition is that when $\delta$ is large, the only thing that counts is that there are $\alpha + 1$ intervals above $b$ and $(\gamma - 1) + 1$ intervals below $b$ (because $c$ is stuck at the bottom, because for $c' \in C \setminus \set{c}$, the likelihood that $c \ppref c'$ vanishes when $\delta$ grows). On average, the objects in $D$ will be spread equally among these intervals, and the limit probability looked for is therefore the proportion of intervals below $b$.
	
	We now determine $P[z < b \knowing {\ppref}]$ for a fixed value of $\delta$.
	We can restrict to considering the complete orderings $\pref$ where the objects composing $A$ and $D$ conform to an arbitrary fixed order $a_1 \pref a_2 \pref …$ and $d_1 \pref d_2 \pref …$. Let us condition on $i$, the number of alternatives among $D$ below $b$ in $\pref$. 
	
	To count the number of orderings $\pref$ extending $\ppref$ such that $i$ objects among $D$ are below $b$, $\card{\downa[b] \cap D} = i$, observe that there are $\binom{i + \gamma}{\gamma - 1}$ ways of placing $D \setminus \set{c}$ among the $\card{\downa[b]} = i + \gamma$ objects below $b$; and there are $\binom{\alpha + (\delta - i)}{\alpha}$ ways of ordering the objects in $A \cup (\upa[b] \cap D)$ above $b$.
	
	Note also that $P[z < b \knowing {\ppref}, \card{\downa[b] \cap D} = i] = \frac{(i + \gamma) + 1}{(\alpha + 1 + \gamma + \delta) + 1}$.
	
	Therefore, we obtain:
	\begin{equation}
		P[z < b \knowing {\ppref}] = \frac{
			\sum_{0 ≤ i ≤ \delta}
			(i + \gamma + 1) \binom{i + \gamma}{\gamma - 1} \binom{\alpha + \delta - i}{\alpha}
		}{
			(\alpha + \gamma + \delta + 2) \sum_{0 ≤ i ≤ \delta} \binom{i + \gamma}{\gamma - 1} \binom{\alpha + \delta - i}{\alpha}
		}.
	\end{equation}
	
	Note that:
	\begin{align}
		(i + \gamma + 1) \binom{i + \gamma}{\gamma - 1}
		& = (i + \gamma + 1) \frac{(i + \gamma)!}{(\gamma - 1)! (i + 1)!}\\
		& = \frac{(i + \gamma + 1)!}{(\gamma - 1)! (i + 1)!}\\
		& = \frac{\gamma (i + \gamma + 1)!}{\gamma ! (i + 1)!}\\
		& = \gamma \binom{i + \gamma + 1}{\gamma}.
	\end{align}
	(The derivation is not valid if $\gamma = 1$ but the conclusion holds in that case as well.)
	...
	\begin{multline}
		\begin{aligned}
		\sum_{0 ≤ i ≤ \delta} \binom{i + \gamma}{\gamma - 1} \binom{\alpha + \delta - i}{\alpha}
		& = \sum_{0 ≤ i ≤ \delta} \binom{i + 1 + \gamma - 1}{\gamma - 1} \binom{\alpha + \delta + 1 - i - 1}{\alpha}\\
		& = \sum_{0 ≤ i ≤ \delta} \binom{i + 1 + \gamma - 1}{i + 1} \binom{\alpha + (\delta + 1) - (i + 1)}{(\delta + 1) - (i + 1)}\\
		& = \sum_{1 ≤ k ≤ \delta + 1} \binom{k + \gamma - 1}{k} \binom{\alpha + (\delta + 1) - k}{(\delta + 1) - k}\\
		& = \sum_{1 ≤ k ≤ \delta + 1} \binom{\gamma + k - 1}{k} \binom{\alpha + 1 + (\delta + 1) - k - 1}{(\delta + 1) - k}\\
		\end{aligned}\\
		\begin{aligned}
		& = \binom{\delta + 1 + \alpha + 1 + \gamma -1}{\delta + 1}
		- \binom{\gamma - 1}{0} \binom{\alpha + 1 + (\delta + 1) - 1}{(\delta + 1)}\\
		& = \binom{\alpha + \gamma + \delta + 1}{\delta + 1}
	...
	\begin{align}
		...
		\frac{
			\sum_{0 ≤ i ≤ \delta}
			\gamma \binom{i + \gamma + 1}{\gamma} \binom{\alpha + \delta - i}{\alpha}
		}{
			\binom{\alpha + \gamma + \delta + 1}{\delta + 1} - \binom{\alpha + \delta + 1}{\delta + 1}
		}\\
		& = \frac{\gamma}{\alpha + \gamma + \delta + 2} 
		\frac{
			\sum_{0 ≤ i ≤ \delta}
			\binom{i + \gamma + 1}{\gamma} \binom{\alpha + \delta - i}{\alpha}
		}{
			\binom{\alpha + \gamma + \delta + 1}{\delta + 1} - \binom{\alpha + \delta + 1}{\delta + 1}
		}\\
		& = \frac{\gamma}{\alpha + \gamma + \delta + 2} 
		\frac{
			\binom{\alpha + \gamma + \delta + 2}{\delta + 1}
			- \binom{\alpha + \delta + 1}{\delta + 1}
		}{
			\binom{\alpha + \gamma + \delta + 1}{\delta + 1} - \binom{\alpha + \delta + 1}{\delta + 1}
		}.
	...
	From there, it must be possible to compute the limit when $\delta$ grows, but I’ll stop here as this approach is probably much more complicated than necessary.
\end{proof}

\begin{conjecture}
	\label{th:infP}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$ and $b \in X$, the worst possible configuration for any partial order with $\card{\upa[b]} = \alpha$ and $\card{\downa[b]} = \gamma$ is the one described in \cref{th:limDelta}. In other words, $\forall {\ppref} \suchthat \card{\upa[b]} = \alpha, \card{\downa[b]} = \gamma$:
	\begin{equation}
		P[z < b \knowing {\ppref}] ≥ \frac{\gamma}{\gamma + \alpha + 1}.
	\end{equation}
...
Given ${\ppref} \in \POs$ and $a \in \allalts$, define $\zeta({\ppref}, a) \in \R^2$ as the vector $(s, -d)$ representing the potential of a question involving $a$ in $\ppref$, namely, $s = (\upa + \downa)$ and $d = \abs{\upa - \downa}$. Note that $s ≥ d$.
...
\begin{conjecture}
	...
	for any $z \in X$ not appearing in $\ppref$,
...
\begin{proof}
	...
	Consider any $s, -d \in \N$, and let us evaluate lower and upper bounds on the gain of a question about $b$ for a partial order with potential $\zeta({\ppref}, b) = (s, d)$.
	
	Consider any ${\ppref} \in \POs$ and $b \in \allalts$ with potential $\zeta({\ppref}, b) = (s, -d)$.

	Define ${\ppref^\text{up}}$ as ${\ppref^\text{up}} = {\ppref} ⇔ \upa[b] ≥ \downa[b]$ and ${\ppref^\text{up}} = {\pprefinv}$ otherwise, 
	and define $M = \frac{s + d}{2} = \max(\card{\upa[b]}, \card{\downa[b]})$ and $m = \frac{s - d}{2} = \min(\card{\upa[b]}, \card{\downa[b]})$,
	so that ${\ppref^\text{up}}$ has $M$ objects above $b$ and $m$ objects below $b$, with $M ≥ m$.
	
	A question $q_{bz}$ yields as expected gain $E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = E[g^{q_{bz}, {\ppref}^\text{up}} \knowing {\ppref^\text{up}}] = P[z < b \knowing {\ppref^\text{up}}] M + P[z > b \knowing {\ppref^\text{up}}] m$.
	Observe that minimizing (respectively, maximizing) $p = P[z < b \knowing {\ppref^\text{up}}]$ is equivalent to minimizing (respectively, maximizing) the gain, when $M$ and $m$ are fixed.
	
	\Cref{th:infP} yields that $p ≥ \frac{m}{m + M + 1}$.
	If this conjecture is correct, we obtain, $\forall {\ppref} \in \POs, b \in \allalts \suchthat \zeta({\ppref}, b) = (s, -d)$:
	\begin{align}
		E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] ≥ \frac{m}{m + M + 1} M + \frac{M + 1}{m + M + 1} m
			& = m \frac{2 M + 1}{m + M + 1}\\
			& = \frac{s - d}{2} \frac{s + d + 1}{s + 1}.
	\end{align}
	If $m = 0$, the formula given in \cref{th:infP} cannot be used, but the bound is trivially valid. 
	
	For the upper bound, note that maximizing $p$, hence, the gain, is equivalent to minimizing $p' = 1 - p = P[z > b \knowing {\ppref^\text{up}}] = P[z < b \knowing {\pprefinv^\text{up}}]$, where ${\pprefinv^\text{up}} = {\ppref^\text{down}} = ({\ppref^\text{up}})^{-1}$ is the inverse of ${\ppref^\text{up}}$, that is, a partial order equivalent to $\ppref$ except that it is possibly inverted so as to have $M$ objects below $b$ and $m$ objects above $b$. By a symmetrical reasoning, we thus obtain, $\forall {\ppref} \in \POs, b \in \allalts \suchthat \zeta({\ppref}, b) = (s, -d)$:
	\begin{align}
		E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] ≤ \frac{m + 1}{m + M + 1} M + \frac{M}{m + M + 1} m
			& = M \frac{2 m + 1}{m + M + 1}\\
			& = \frac{s + d}{2} \frac{s - d + 1}{s + 1}.
	\end{align}
	If $M = 0$, the formula given in \cref{th:infP} cannot be used, but the bound is trivially valid as $M = 0 ⇒ m = 0 ⇒ E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = 0$.
	\footnote{As a sanity check, consider $M = \alpha = 3, m = \gamma = 1$, then given $a_1 a_2 a_3 b c$ with objects above $c$, $P[z < b]$ tends to and is greater than $1/5$, thus the gain is greater than $\frac{1}{5} 3 + \frac{4}{5} 1 = \frac{m}{m + M + 1} M + \frac{M + 1}{m + M + 1} m$; and considering the inverse ordering $a b c_1 c_2 c$ with objects above $c$, we see that $P[z < b]$ tends to and is greater than $3/5$, thus the gain is smaller than $\frac{3}{5} 1 + \frac{2}{5} 3 = \frac{M}{m + M + 1} m + \frac{m + 1}{m + M + 1} M$.
...
\begin{remark}
	Define $s = 6 > s' = 4$, $d = d' = 2$, $(M, m) = (4, 2)$, $(M', m') = (3, 1)$.
	
	Now let’s try to find low $P[z < b \knowing {\ppref}]$ with $\alpha = 4, \gamma = 2$ and high $P[z < b \knowing {\ppref'}]$, that is, low $P[z > b \knowing {\ppref'}] = P[z < b \knowing {\ppref^{\prime -1}}]$ with $\alpha' = 3, \gamma' = 1$ thus $\alpha^{\prime\prime} = 1, \gamma^{\prime\prime} = 3$ corresponding to ${\ppref^{\prime\prime}} = {\ppref^{\prime -1}}$.
	
	I can make $P[z < b \knowing {\ppref}] = \frac{2}{7} = \frac{10}{35}$ and $P[z < b \knowing {\ppref^{\prime -1}}] = 3 / 5$ thus $P[z > b \knowing {\ppref^{\prime - 1}}] = 1 - 3 / 5 = \frac{2}{5} = \frac{14}{35} = P[z < b \knowing {\ppref'}]$.
	
	Then $E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = \frac{2}{7} 4 + \frac{5}{7} 2 = \frac{18}{7} = \frac{90}{35}$, whereas $E[g^{q_{bz}, {\ppref'}} \knowing {\ppref'}] = \frac{2}{5} 3 + \frac{3}{5} 1 = \frac{9}{5} = \frac{63}{35}$.
	
	In general: $P[z < b \knowing {\ppref}] = \frac{m}{M + m + 1}$ and $P[z < b \knowing {\ppref^{\prime -1}}] = \frac{\gamma^{\prime\prime}}{\alpha^{\prime\prime} + \gamma^{\prime\prime} + 1} = \frac{M'}{M' + m' + 1}$ thus $P[z > b \knowing {\ppref^{\prime - 1}}] = 1 - \frac{M'}{M' + m' + 1} = \frac{m' + 1}{M' + m' + 1} = P[z < b \knowing {\ppref'}]$.
	
	Then $E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = \frac{m}{M + m + 1} M + \frac{M + 1}{M + m + 1} m = \frac{18}{7} = \frac{90}{35}$, whereas $E[g^{q_{bz}, {\ppref'}} \knowing {\ppref'}] = \frac{m' + 1}{M' + m' + 1} M' + \frac{M'}{M' + m' + 1} m' = \frac{9}{5} = \frac{63}{35}$.
	
	For example, with $s = s' = 1000$, $d = 2$, $d' = 4$, thus $(M, m) = (501, 499)$ and $(M', m') = (502, 498)$, we obtain 
	$E[g^{q_{bz}, {\ppref}} \knowing {\ppref}] = \frac{499}{1001} 501 + \frac{502}{1001} 499 = \frac{500497}{1001}$, whereas
	$E[g^{q_{bz}, {\ppref'}} \knowing {\ppref'}] = \frac{498}{1001} 502 + \frac{503}{1001} 498 = \frac{500490}{1001}$.
	
\end{remark}

\begin{remark}[Simple preorders have right probs]
	Claim: $s > s' ⇒ P[z < b \knowing {\ppref}] ≥ P[z < b \knowing {\ppref'}]$.
	
	When nothing else, the claim is true:
	\begin{align}
		P[z < b \knowing {\ppref}] & ≥ P[z < b \knowing {\ppref'}]\\
		⇔ \frac{m + 1}{s + 2} & ≥ \frac{m' + 1}{s' + 2}\\
		⇔ \frac{s - d + 2}{s + 2} & ≥ \frac{s' -d + 2}{s' + 2}\\
		⇔ (s - d + 2)(s' + 2) & ≥ (s' -d + 2)(s + 2)\\
		⇔ ss' - ds' + 2s' + 2s - 2d + 4 & ≥ ss' - ds + 2s + 2s' - 2d + 4\\
		⇔ -ds' & ≥ -ds\\
		⇔ s' & ≤ s.
	\end{align}
\end{remark}
...
\begin{remark}
	Define $s = 5 > s' = 3$, $d = d' = 1$, $(M, m) = (3, 2)$, $(M', m') = (2, 1)$.
	${\ppref} = \set{a_1, a_2, a_3} \ppref b \ppref \set{c_1, c_2}$…
	${\ppref'} = \set{a_1, a_2} \ppref b \ppref \set{c_1}$…
	
	Now let’s try to find $\ppref$ with $(M, m) = (3, 2)$ and with low $P[z < b \knowing {\ppref}]$ and $\ppref'$ with $(M', m') = (2, 1)$ and with high $P[z < b \knowing {\ppref'}]$.
	
	I can make $P[z < b \knowing {\ppref}] = 1/3$ and $P[z < b \knowing {\ppref^{\prime -1}}] = 1/2$ thus $P[z > b \knowing {\ppref^{\prime - 1}}] = 1 - 1/2 = 1/2 = P[z < b \knowing {\ppref'}]$.

	Contradicts claim: $P[z < b \knowing {\ppref}] ≥ P[z < b \knowing {\ppref'}]$.
\end{remark}
