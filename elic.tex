...
\begin{figure}
	\begin{tikzpicture}
		...
		\path (tree0.south) ++ (0, -1cm) node[anchor = north] (root abc) {$a > b > c$}
		...
		;
		\begin{scope}
			\path (root abc-1) ++ (offsetL) ++ (0mm, 7mm) node (a) {$a$} ++(offsetG) node (b) {$b$} ++(offsetG) node (c) {$c$} ++ (offsetG) node (d) {$d$};
			\path graph[use existing nodes] {a -- b -- c -- d};
		\end{scope}
%		\path[anchor=base] (root abc-1) ++ (offsetR) ++ (-1mm, 4mm) node {$a$} ++(offsetG) node {$b$} ++(offsetG) node {$c$} ++(3mm, 4mm) node {$d$};
%		\begin{scope}[shift = {($(root abc-1) + (offsetL) + (0mm, 7mm)$)}]
%			\path graph[branch down = 4mm, math nodes] {
%				a,
%				b,
%				c,
%				d
...
%			\path graph[use existing nodes] {a -- b -- c -- d};
%		\end{scope} 
%		\begin{scope}[shift = {($(root abc-1) + (offsetR) + (0mm, 7mm)$)}]
%			\path graph[branch down = 4mm] {
%				a,
%				b d,
%				c
%			};
%		\end{scope} 
		...
		\path (root abc) ++ (0, -6cm) [sibling distance = 3cm, level 2/.style = {sibling distance = 1.5cm, align = left, anchor = north}] node[anchor=north] (root ac b) {$\set{a, c} > b$}
			child {
				node {$a \relq c$}
				child {
					node {
