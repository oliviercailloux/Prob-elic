...
\begin{figure}
	...
	\begin{tikzpicture}
		\path node (aba) {$a$};
		\path (aba.south) ++(0, -\ordown) node[anchor=north] (abb) {$b$};
		\path [draw] (aba) -- (abb);
		\path [fit={(aba) (abb)}] coordinate (ab);

		\path (abb.south) ++(-1cm, -1cm) node[anchor=north] (abca) {$a$};
		\path (abca.south) ++(0, -\ordown) node[anchor=north] (abcb) {$b$};
		\path (abcb.south) ++(0, -\ordown) node[anchor=north] (abcc) {$c$};
		\path [draw] (abca) -- (abcb) -- (abcc);
		\path [fit={(abca) (abcb) (abcc)}] coordinate (abc);
	\end{tikzpicture}
