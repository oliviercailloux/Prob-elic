...
\begin{conjecture}
	...
	Define $A = {\pprefinv}(b) = \upb$, $C = {\ppref}(b) = \downb$, $D ={\pprefeq^{±1}}[b] \setminus {\pprefeq^{±1}}(b)$, 
	$\alpha = \card{A}$, $\gamma = \card{C}$ and $\delta = \card{D}$.
...
\begin{proof}
	...
	Observe that minimizing (respectively, maximizing) $p$ is equivalent to minimizing (respectively, maximizing) the gain, when $M$ and $m$ are fixed.
