...
\begin{figure}
	\begin{tikzpicture}
		...
		\path (ab-b.south)  edge node[above left] {$b > c$} (abc);
		...
		\path (abc-bd-c.base) ++(6mm, 0) node[anchor=base, inner sep=0] (abc-bd-d) {$d$};
		\path [draw] (abc-bd-a) -- (abc-bd-b) -- (abc-bd-c);
		\path [draw] (abc-bd-b) -- (abc-bd-d);

		\path (abc-c.south) ++(1cm, -1cm) coordinate (abc-db);
		\path (abc-db) ++(-3mm, 0) node[anchor=north] (abc-bd-a) {$a$};
		...
		\path (ab-cb-south)  edge node[above left] {$b > d$} (abd-cb);
		\path (ab-cb-south) edge node[above right] {$d > b$} (ab-cb-db);		
	\end{tikzpicture}
	\caption{An optimal strategy for three questions}
	\label{fig:k3a}
\end{figure}
