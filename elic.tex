...
\end{proof}
By symmetry, a similar results holds for lower contour sets.
...
\begin{proposition}
	Given ${\ppref} \in \POs$ and $q_1, q_2, q'_1, q'_2 \in \allalts$, if ${\ppref} + (q_1, q_2) = {\ppref} + (q'_1, q'_2)$, then $q_1 = q'_1 \land q_2 = q'_2$.
\end{proposition}
\begin{proof}
	...
	Note that $T({\ppref} \cup \set{(i, j)}) = {\ppref} \cup \set{(i', j') \suchthat i' \pprefeq i \land j \pprefeq j'}$.
Therefore:
\begin{equation}
	\label{eq:trans}
	T({\ppref} \cup \set{(i, j)}) \setminus {\ppref} \subseteq \set{(i', j') \suchthat [(i' \pprefeq i) \land (j \pprefeq j')]}.
\end{equation}

	$(q_1, q_2) \in {\ppref'}$, $(q'_1, q'_2) \in {\ppref'}$, $(q_1, q_2) \notin {\ppref}$, $(q'_1, q'_2) \notin {\ppref}$.
	...
	\commentOC{TODO adapt to new notations and improve.}
\end{proof}
...
\section{Probability distributions over the preferences and the questions}
Define a questioning strategy as a function $s$ that given ${\ppref} \in \POs$ returns a probability distribution over the set of possible questions $Q$.

Together with the probability distribution $P$ over the possible preferences, this defines a joint probability distribution $P^s: \powerset{\linors × Q^{\card{Q}}} → [0, 1]$ over the possible preferences and questions asked.

Thus, if $\card{Q} = 3$, $P(\pref, q_1, q_2, q_3)$ denotes the probability that the preference is $\pref$ and that the questions asked were $q_1$, then $q_2$, and finally $q_3$.

Given $k \in \N$, let $P(\set{(\pref, (\ppref_t)_{t \in \intvl{1, k}})}) = P(\set{(\pref, (q_t)_{t \in \intvl{1, k}}) \suchthat \forall t \in \intvl{1, k}: {\ppref_t} = {\ppref_{t - 1}} + q_t})$ denote the probability that the preference is $\pref$ and that the questions asked lead to the sequence of graphs $(\ppref_t)_{t \in \intvl{1, k}}$.
Example: $P(\ppref_1 = \set{(a, b)}) = P(q_1 = q_{ab})$.
Example: $P(\ppref_2 = \set{(a, b), (c, b)}) = P\big((\set{q_1, q_2} = \set{q_{ab}, q_{bc}}) \text{ and } a \pref b \text{ and } c \pref b\big)$.

Given $i, j \in \allalts$, define $P_t((i, j)) = P(i \ppref_t j)$ as the probability that the edge $(i, j)$ be known after $t$ questions.
