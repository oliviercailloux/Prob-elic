...
\begin{proposition}
	Given ${\prefeq} \in \linors$, $k \in \N^*$ and $(q_t)_{t \in \intvl{1, k}} \in Q^k$, $P^s(\prefeq, (q_t)_{t \in \intvl{1, k}}) = P^{\prime s}(\prefeq, \set{(q'_t)_{t \in \intvl{1, k'}} \in Q^{\prefeq} \suchthat k' ≥ k \land \restr{q'_t}{\intvl{1, k}} = q_t})$, the probability that the real preference be $\prefeq$ and that the strategy $s$ asks any sequence of questions in $Q^{\prefeq}$ that completes $q_t$.
\end{proposition}
\begin{proof}
...
\begin{remark}
	...
	\begin{itemize}
		...
		\item $P^s(q_2 | {\pref}, q_1) = P^s({\pref}, q_1, q_2) / P^s({\pref}, q_1) = P^s({\pref}, q_1, q_2) / [s(\emptyset)(q_1)P({\pref})] = s(\emptyset)(q_1) s(>_1)(q_2) / s(\emptyset)(q_1) = s(>_1)(q_2).$
	\end{itemize}
\end{remark}
...
The optimal $k$-strategies knowing $\ppref$ are defined as the strategies maximizing $E^s[\card{{\ppref}_k} \knowing {\ppref}] = \sum_{{\ppref}_k \supseteq {\ppref}_{k - 1} \subseteq … \subseteq {\ppref}_1 \subseteq {\ppref}} P^s({\ppref}_k \knowing {\ppref}_{k - 1})$.
