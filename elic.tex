...
\begin{conjecture}
	\label{th:infP}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$ and $b \in X$, the configuration that minimizes $P[z < b \knowing {\ppref}]$, for any partial order with $\card{\upb} = \alpha$, $\card{\downb} = \gamma$ and $\delta$ other objects in the component containing $b$, is the one described in \cref{th:limDelta} with $\gamma' = \gamma$. Furthermore, in the case $\gamma' = \gamma$, $P[z < b \knowing {\ppref}]$ can be written $\frac{\delta + (\alpha + 2) (\gamma + 1)}{(\alpha + \gamma + \delta + 2) (\alpha + 2)}$.
	In summary, $\forall {\ppref} \suchthat \card{\upb} = \alpha, \card{\downb} = \gamma$ with $\delta$ other objects in the component containing $b$:
	\begin{equation}
...
\begin{conjecture}
	...
	Define $A = \upb$, $C = \downb$, $A' = \upb'$ and $C' = \downb'$; and
	$\alpha = \card{A}$, $\gamma = \card{C}$, $\alpha' = \card{A'}$ and $\gamma' = \card{C'}$.
	
	Let ${\pprefeq^{±1}}[b]$ denote the connected component containing $b$ in ${\pprefeq}$, thus, the transitive closure of $\pprefeq$ and $\pprefeqinv$ starting from $b$. Let $D = {\pprefeq^{±1}}[b] \setminus A \setminus \set{b} \setminus C$ denote the elements path-connected to $b$ but incomparable to $b$ in $\ppref'$, and define $\delta = \card{D}$. Define similarly $D'$ and $\delta'$ wrt $b'$ and $\ppref'$.
	...
	\begin{equation}
		...
		⇒ E ≥ E'.
	\end{equation}
	Note that the condition is satisfied whatever $\delta, \delta'$ when $\alpha = \gamma = 1$.
\end{conjecture}
\begin{proof}
	I am interested in analyzing the case $\delta = \delta'$, for simplicity.
	
	Let us compute the lowest possible $E$. That is, $E = P[z < b \knowing {\ppref}] \alpha + P[z > b \knowing {\ppref}] \gamma$ with the lowest possible $P[z > b \knowing {\ppref}] = P[z < b \knowing {\pprefinv}]$. Thus:
	\begin{equation}
		P[z < b \knowing {\pprefinv}] = \frac{\delta + (\gamma + 2) (\alpha + 1)}{(\gamma + \alpha + \delta + 2) (\gamma + 2)}.
	\end{equation}
	
	We want to compare this to the highest possible $E'$. That is, $E' = P[z' < b' \knowing {\ppref'}] \alpha' + P[z' > b' \knowing {\ppref'}] \gamma'$ with the smallest possible $P[z' < b' \knowing {\ppref'}]$, thus:
	\begin{equation}
		P[z' < b' \knowing {\ppref'}] = \frac{\delta + (\alpha' + 2) (\gamma' + 1)}{(\alpha' + \gamma' + \delta + 2) (\alpha' + 2)}.
	\end{equation}
...
\begin{proof}
	...
	and define $M = \frac{s + d}{2} = \max(\card{\upb}, \card{\downb})$ and $m = \frac{s - d}{2} = \min(\card{\upb}, \card{\downb})$,
