...
\begin{proof}
	...
	To count the number of orderings $\pref$ extending $\ppref$ such that $i$ objects among $D$ (say, $D_1 = \downa[b] \cap D$) are below $b$, $\card{D_1} = i$, observe that there are $\binom{i + \gamma}{\gamma - \gamma'}$ ways of placing $C \setminus C'$ (the objects among $C$ whose position relative to $D$ is not determined) among the $\card{\downa[b]} = i + \gamma$ objects below $b$; and there are $\binom{\alpha + (\delta - i)}{\alpha}$ ways of ordering the objects in $A \cup (\upa[b] \cap D)$ above $b$.
...
\begin{conjecture}
	\label{th:infP}
	Given $\alpha, \gamma \in \N, \gamma ≥ 1$ and $b \in X$, the configuration that minimizes $P[z < b \knowing {\ppref}]$, for any partial order with $\card{\upa[b]} = \alpha$, $\card{\downa[b]} = \gamma$ and $\delta$ other objects in the component containing $b$, is the one described in \cref{th:limDelta} with $\gamma' = \gamma$. Furthermore, in the case $\gamma' = \gamma$, $P[z < b \knowing {\ppref}]$ can be written $\frac{\delta + (\alpha + 2) (\gamma + 1)}{(\alpha + \gamma + \delta + 2) (\alpha + 2)}$.
	In summary, $\forall {\ppref} \suchthat \card{\upa[b]} = \alpha, \card{\downa[b]} = \gamma$ with $\delta$ other objects in the component containing $b$:
	\begin{equation}
		P[z < b \knowing {\ppref}] ≥ \frac{\delta + (\alpha + 2) (\gamma + 1)}{(\alpha + \gamma + \delta + 2) (\alpha + 2)}.
	\end{equation}
\end{conjecture}
\begin{proof}[Partial]
	Let’s admit for now that the minimizing configuration is the one of \cref{th:limDelta} with $\gamma = \gamma'$.
