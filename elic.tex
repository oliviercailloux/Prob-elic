...
\begin{proof}
	...
	Define $s$ as the questioning strategy that asks $q_{bf}$, then $q_{bg}$, …, with a new element at each step.
	...
	That is because $s'$ asks some question $q_{xy}$ not related to $a$ ($x ≠ a$ and $y ≠ a$). Build $t$ by replacing one of these questions with a question about $a$ ($q_{ax}$ or $q_{ay}$). This can only improve the expected gain. 
...
\begin{remark}
	...
	I’d guess having $(a, g)$ is better. 
	Thus, thm: $E_a > E_b$.
	This seems true here. 
