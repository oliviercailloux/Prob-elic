...
\begin{proposition}[draft!]
	...
	\[P(a > b \knowing {\ppref}) = \frac{1 + {\ppref^{-1}}(b)}{1 + {\ppref^{-1}}(b) + 1 + {\ppref}(b)}.\]
	In particular, it is independent of the relative ordering of the elements below $b$.
	\footnote{This is also equal to:
		\[P(a > b \knowing {\ppref}) = \frac{(1 + {\ppref^{-1}}(b))! {\ppref}(b)!}{(1 + {\ppref^{-1}}(b))! {\ppref}(b)! + {\ppref^{-1}}(b)! (1 + {\ppref}(b))!}.\] 
	}
\end{proposition}
\begin{proof}
...
\begin{proof}
	...
	\[E[\card{T({>} \cup (\pref \cap q_{af}))} - \card{{>}} - 1 \knowing >] = P(f > a \knowing >) (\card{{\ppref}(b)} + 1),\]
	\[E[\card{T({>} \cup (\pref \cap q_{bf}))} - \card{{>}} - 1 \knowing >] = P(f > b \knowing >) \card{{\ppref}(b)} + P(b > f \knowing >) \card{{\ppref}^{-1}(b)}.\]
	Thus, we need to prove:
	\[P(f > b \knowing >) \card{{\ppref}(b)} + P(b > f \knowing >) \card{{\ppref}^{-1}(b)} > P(f > a \knowing >) \card{{\ppref}(b)} + P(f > a \knowing >),\]
	or equivalently:
	\[[P(f > b \knowing >) - P(f > a \knowing >)] \card{{\ppref}(b)} > P(f > a \knowing >) + P(f > b \knowing >) \card{{\ppref}^{-1}(b)} − \card{{\ppref}^{-1}(b)},\]
	or equivalently:
	\[P(a > f > b \knowing >) \card{{\ppref}(b)} > P(f > a \knowing >) + P(f > b \knowing >) \card{{\ppref}^{-1}(b)} − \card{{\ppref}^{-1}(b)}.\]
	
	Consider $x_1 = a$ to $x_k$ above $b$ above $\card{{\ppref}(b)}$ elements. 
	$P(f > a \knowing \ppref, \card{{\pprefeqinv}(a)} = r) = r / (k + 2 + \card{{\ppref}(b)})$. 
	Thus, $P(f > a \knowing \ppref) = (1 + k) / 2 / (k + 2 + \card{{\ppref}(b)})$.
	Whereas $P(f > b \knowing \ppref) = (1 + k) / (k + 2 + \card{{\ppref}(b)})$.
	Thus, the inequality holds.
	
	
	\footnote{I thought that this would be true even when replacing $\card{{\ppref}^{-1}(b)}$ with 1, a proof of which would suffice to prove the claim (because this replacement is unfavorable to $E[q_{bf}]$). Because $P(f > a \knowing >) + P(f > b \knowing >) − 1 < 2 P(f > b \knowing >) − 1$, we would thus have to prove:
	$[P(a > f > b \knowing >)] \card{{\ppref}(b)} ≥ 2 P(f > b \knowing >) − 1$. But this is false. Consider $a, d, e, g$ better than $b$ better than $c$. $P(f > b \knowing >) = 5/7$. RHS = $3/7$ > LHS = $P(a > f > b \knowing >) = 2.5 / 7$.}
