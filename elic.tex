...
\begin{proof}
	...
	Let us examine the more general formula, for $\gamma' \in N, \gamma' ≤ \gamma$:
	\begin{equation}
		P[z < b \knowing {\ppref}] = \frac{
			\sum_{0 ≤ i ≤ \delta}
	...
	\end{align}
	\commentOC{A numerical check suggests that there might be a mistake in the second term.}
	
	In particular, for $\gamma' = 1$, $R(\gamma) = R(\gamma + 1) = \binom{\alpha + \delta + 1}{\delta + 1}$, hence:
	\begin{align}
		P[z < b \knowing {\ppref}] 
...
\begin{proof}
	...
	To count the number of orderings $\pref$ extending $\ppref$ such that $i$ objects among $D$ (say, $D_1 = \downa[b] \cap D$) are below $b$, $\card{D_1} = i$, observe that there are $\binom{i + \gamma}{\gamma - \gamma}$ ways of placing $C \setminus C$ (the objects among $C$ whose position is not determined) among the $\card{\downa[b]} = i + \gamma$ objects below $b$; and there are $\binom{\alpha + (\delta - i)}{\alpha}$ ways of ordering the objects in $A \cup (\upa[b] \cap D)$ above $b$.
	...
	\begin{align}
		...
		= {}& \frac{\delta + (\alpha + 2) (\gamma + 1)}{(\alpha + \gamma + \delta + 2) (\alpha + 2)}
		 \frac{\binom{\alpha + \delta + 1}{\delta} \frac{1}{\alpha + 2}}{\binom{\alpha + \delta + 1}{\delta} \frac{1}{\alpha + 2}}.
	\end{align}
\end{proof}

\begin{remark}
	Consider $\alpha = 0$, $\gamma = 4$, $\delta = t$ and $\ppref$ with this potential but highest possible gain. That is, $E = P[z > b \knowing {\ppref}] 4$ with the smallest possible $P[z < b \knowing {\ppref}]$, thus:
	\begin{equation}
		P[z < b \knowing {\ppref}] = \frac{\delta + (\alpha + 2) (\gamma + 1)}{(\alpha + \gamma + \delta + 2) (\alpha + 2)}.
	\end{equation}
	
	Compare this to $\alpha' = 1$, $\gamma' = 3$, $\delta' ≤ t$ and $\ppref'$ with this potential but lowest possible gain. That is, $E' = P[z < b \knowing {\ppref'}] 1 + P[z > b \knowing {\ppref'}] 3$ with the highest possible $P[z < b \knowing {\ppref'}]$.
	
	I want to find the greatest $t$ such that $E' ≥ E$.
