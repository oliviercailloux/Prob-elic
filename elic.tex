...
\begin{proof}
	...
	Now consider $a > d$, $abc$.
	
	
\end{proof}

\begin{remark}
	It may be thought that in every configurations where $b$ is in the middle of every paths of length 3, with no paths of length more than 3 and at least one path of length 3, it holds that asking about $b$ is best. This is false. 
	
	Let $a \ppref b$, $A = {\pprefinv}(b)$, $C = {\ppref}(b)$, S = {\ppref}(a) \setminus \set{b} \setminus C$.
	
	$P(f > b \knowing {\ppref}) = \sum_{k = 0}^{\card{S}} [(\card{S} - k) + \card{A} - 1]! \binom{\card{S}}{\card{S} - k} (k + \card{C})! (\card{S} - k + \card{A} + 1)$.
	Here, $k$ represents the number of elements of $S$ that are above $b$. There are $[(\card{S} - k) + \card{A} - 1]$ possible permutations for those elements, together with …
\end{remark}
