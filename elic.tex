...
\subsection{Conjecture for 4 questions}
Computer experiments (to be checked) suggest that the following is the strategy that maximizes $E^s[\card{{\ppref}_4}]$ (assuming $n ≥ 5$). 
Start, wlog, by using two questions to order $\set{a, b, c}$. If $>_2$ is a linear order (say, $abc$), spend the remaining two questions by comparing yet unknown elements with $b$. Otherwise (say, $\set{a, b} > c$), disconnect temporarily by asking $q_{de}$, then connect the two components.
