...
Given $i ≠ j \in \allalts$, ${\pref} \cap q_{ij}$ is a singleton that contains, intuitively, the answer to the question $q_{ij}$ when the preference is $\pref$, that is, the edge connecting $i$ to $j$ that is in $\pref$.
...
Define $P^\mathit{eq}$ as the equiprobable distribution over $\linors$, thus, $\forall {\prefeq} \in \linors: P^\mathit{eq}(\prefeq) = 1/\card{\linors}$.
...
Given a partial order $\ppref$, we write $P(\ppref) = P(\set{{\pref} \suchthat {\ppref} \subseteq {\pref}})$ for the probability that the real preference be compatible with $\ppref$.
...
\section{Analysis for 3 questions}
Consider $k = 3$, $n = 4$, and let $s$ denote an optimal strategy.
...
\begin{itemize}
	\item $P(cd \knowing abc) = \frac{P(abcd)}{P(abc)} = \frac{3!}{4!} = 1/4$. Gain: 2. Otherwise, gain: 0. E: 1/2.
