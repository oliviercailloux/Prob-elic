...
Define an addition operation over the possible graphs, representing our increase in knowledge after having obtained an answer to a question: we add the resulting edge, and compute the transitive closure. Formally, given $\ppref$ and a question $q$, ${\ppref} + q(>) = T({\ppref} \cup \set{q(>)})$, where $T$ denotes the irreflexive transitive closure. Example: $\set{(a, b)} + q_{bc}(>) = {>}$.
...
Given $\ppref, \ppref' \in \POs$, say that the transition from $\ppref$ to $\ppref'$ is legal iff $\exists q \in Q \suchthat {\ppref'} = {\ppref} + q(\pref)$.
...
\begin{proposition}
	Given ${\ppref} \in \POs$ and $q, q' \in Q$, if ${\ppref} + q(\pref) = {\ppref} + q'(\pref)$, then $q = q'$.
\end{proposition}
\begin{proof}
	Let $N = ({\pprefeq} + q(\prefeq)) \setminus {\pprefeq}$ be the set of pairs brought thanks to the question $q$. The claim is that $\pprefeq$ and $N$ together determine the question uniquely. Let $q = q_{ij}$. By definition of $+$, $N \subseteq \set{(a, b) \suchthat a ≥ i \land j ≥ b}$. Let $L = N^{-1}(\allalts)$ be the left projection of $N$. 
	
	This approach seems hopeless: the new set is only a subset of an UCS.
\end{proof}
...
%\end{equation}
%
%Say that a question $q \in Q$ may lead from $\ppref_t$ to $\ppref_{t + 1}$ iff $\exists {>} \in \linors \suchthat \ppref_{t + 1} = T(\ppref_t \cup \set{q(>)})$.
%The following proposition shows that only one question, at most, permits to lead from any graph $\ppref_t$ to any graph $\ppref_{t + 1}$. 
%\begin{proposition}
%	Given two possible graphs $\ppref_{t + 1} ≠ \ppref_t$ and any two questions $q, q' \in Q$, if both $q$ and $q'$ may lead from $\ppref_t$ to $\ppref_{t + 1}$, then $q = q'$.
%\end{proposition}
%\begin{proof}
